...
:::
```{r}
data_gapminder_num <- select(gap_dat, is.numeric)
...
- `sapply()` simplifies the output, returning a vector or matrix
...
# Working with `lapply()` and `sapply()`
...
- `mapply()` applies function *across* more than one object
