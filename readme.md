...
# Modeling workshop: fitting your models to data rather than your data to models
...
This series of 30-minute workshops (plus 15 minutes for questions and clarifications) is aimed at introducing early-career researchers to statistical models that extend beyond linear models (i.e., ANOVAs) so that they may learn how to *fit models to their data rather than fitting their data to models*. All workshops will use `R` and RStudio, so some experience with `R` or other programming languages is encouraged but not required. See the [R Fundamentals workshop](https://csc-ubc-okanagan.github.io/workshops/) for an introduction to `R` and RStudio. Attendees who do not have experience with `R` are encouraged to review this content or take the introductory workshop concurrently if it's being offered. Alternatively, you can [book a consultation with a Research Data Consultant](https://csc.ok.ubc.ca/consultations/research-data-consultation/).
